...
public class InvoiceModel {
    ...
    private ArrayList<InvoiceLine> currentSelectedInvoicesLineList;
    ...
    public void readFiles(String headersFilePath, String linesFilePath) throws IOException {
        ...
        linesFile = InvoiceLine.fromCSV(csvLines, headersFile);
                System.out.println("Invoices Header File"+"\n"+"****************************************");
    ...
    private void fillHeadersTableModel() {
        if (headersFile != null) {
            invTableModel.setRowCount(0); //Resetting the table model
            for (InvoiceHeader header : headersFile) {
    ...
    public void createNewItem(InvoiceHeader header, String name, String count, String price) {
        if (linesFile != null) {
            ...
            ArrayList<InvoiceLine> currentLines = header.getLines();
            currentLines.add(line);
            header.setLines(currentLines);
            refreshInvoicesTableAndItemsTable();
    ...
    public void deleteItem(int index) {
        ...
        InvoiceHeader currentHeader = currentSelectedInvoiceLine.getHeader();
        linesFile.remove(currentSelectedInvoiceLine);
        ArrayList<InvoiceLine> currentLines = currentHeader.getLines();
        currentLines.remove(currentSelectedInvoiceLine);
        currentHeader.setLines(currentLines);
        refreshInvoicesTableAndItemsTable();
    }

    private void refreshInvoicesTableAndItemsTable() {
        itmTableModel.fireTableDataChanged();
        invTableModel.fireTableDataChanged();
        fillHeadersTableModel();
    }
